...
export function SubKegiatanFormModal({ mode, initialData, onClose }: Props) {
  ...
  }, [isLevel1, isKegiatanOrDeeper, form.isWadah]);
  ...
  function handleSave() {
    if (mode === 'add') {
      ...
      if (newInputMode === 'kegiatan' && newInputValue.trim() !== '') {
        alert("Silakan klik tombol centang (✓) berwarna biru di sebelah input Kegiatan untuk menyimpan nama kegiatan baru terlebih dahulu.");
        return;
      }
      if (!form.kegiatanId) {
