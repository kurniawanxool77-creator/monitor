...
function getDynamicConfig(name = '') {
...
export function ProgressSubKegiatan() {
  ...
  const [selectedBagian, setSelectedBagian] = useState(CARDS_ORDER.length > 0 ? CARDS_ORDER[0] : 'Sekretariat DPRD');
  ...
  function toggleStep(subKegiatanId, stepId) {
  ...
  const sortedKegiatans = [...filteredKegiatans].sort((a, b) => {
    const priority = { 'Terlambat': 1, 'Belum Mulai': 2, 'Berjalan': 3, 'Selesai': 4 };
